...
浏览器加载 index.html
  └─ <script src="script.js">
       ├─ 创建 Worker("onnx-worker.mjs", {type:"module"})
       │   └─ Worker 内部: import ort from node_modules
       ...
       └─ init()                                      async 入口
            ...
            ├─ await workerInit()                     Worker 加载 7 个 ONNX 模型
...
| 启动时间 | 数百毫秒（需 `npm install` + 加载 7 个 .onnx + WASM runtime）    |
| 函数调用 | JS → postMessage → Worker → `session.run()` → postMessage → JS   |
